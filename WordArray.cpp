...
WordArray &WordArray::operator=(const WordArray &rhs)
{
	if(m_arrayPtr != nullptr) delete[](this->m_arrayPtr);
	this->m_capacity = rhs.m_capacity;
	this->m_count = rhs.m_count;
	this->m_arrayPtr = new std::string[this->m_capacity];
	return *this;
}
WordArray WordArray::operator+(const WordArray &rhs) const
{
	WordArray wordArray{ this->m_capacity + rhs.m_capacity };
	for (size_t i = 0; i < this->m_count; i++)
	{
		wordArray.insertWordAtIndex(this->getWordAtIndex(i), i);
	}
	for (size_t j = 0; j < rhs.m_count; j++)
	{
		wordArray.insertWordAtIndex(rhs.getWordAtIndex(j), j + this->m_count);
	}
	return wordArray;
}
WordArray WordArray::operator+(const std::string &rhs) const
{
	WordArray wordArray{ this->m_capacity };
	if (this->m_count + 1 > this->m_capacity)
	{
		std::cerr << "[ERROR] The array is already full!" << std::endl;
		return *this;	
	}
	for (size_t i = 0; i < this->m_count; i++)
	{
	...
	wordArray.insertWordAtIndex(rhs, this->m_count + 1);
	return wordArray;
}
WordArray WordArray::operator-(const std::string &rhs) const
{
	...
	for (size_t i = 0; i < this->m_count; i++)
	{
		if (this->getWordAtIndex(i) != rhs) wordArray.insertWordAtIndex(this->getWordAtIndex(i), i - offset);
		else offset++;
	}
	return wordArray;
}
bool WordArray::operator==(const WordArray &rhs) const
{
	if (this->m_capacity != rhs.m_capacity || this->m_count != rhs.m_count) return false;
	for (size_t i = 0; i < this->m_count; i++)
	{
		if (this->getWordAtIndex(i) != rhs.getWordAtIndex(i)) return false;
	}
	return true;
}